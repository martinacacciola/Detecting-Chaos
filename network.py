...
# Preprocess function
# TODO:
# not sure this code is taking just the first n_steps or creating sequences of n_steps each
# positions_velocities should be of size (n_steps, 18) - to have these do not group by particle
def preprocess_trajectory(file_path, gaussian_fit_params_path=None, n_steps=1):
    """
    ...
    # Group trajectories by particle and include just the first n_steps of each trajectory
    forward_trajectories = {
        particle: forward_trajectory[forward_trajectory['Particle Number'] == particle][pos_vel_cols].astype(float).to_numpy()[:n_steps]
        for particle in particles
    }

    return forward_trajectories, gaussian_fit_params
...
def build_temporal_encoder(sequence_length, feature_dim, latent_dim):
    ...
    #attention = layers.Attention()(forward_encoded)
    
    # Combine features with learned attention
    #combined_features = layers.Concatenate(name='combined_features')([forward_encoded, attention])
    ...
    aggregated_features = layers.GlobalAveragePooling1D(name="aggregated_features")(forward_encoded)
...
def build_phase_space_model(sequence_length, feature_dim, latent_dim, num_components):
    ...
    latent_representation = encoder(forward_input)
    ...
    model = models.Model(inputs=forward_input, outputs=outputs, name="phase_space_model")
    return model
